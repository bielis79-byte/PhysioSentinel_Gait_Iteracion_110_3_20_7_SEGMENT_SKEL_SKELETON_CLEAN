...
# V110.3.20.7 SEGMENT · SKEL SKELETON LAYER
...
def _refine_fullbody_q_sequence_202(torch,model,sequence,betas,zero_trans,axis_map,scale):
    ...
    return {'applied':True,
            'method':'7-point symmetric FIR + adaptive blend + >=92% ROM preservation + robust bidirectional step limiter + exact frame1 anchor (segmental branch; no q14-specific curvature guard)',
...
def _fit_skel_sequence(torch, model, motion, seed_fit, start_index=0, iterations=16, progress_cb=None, empirical_profile=None, acquisition_mode="monocular_depth_safe"):
    ...
    result={'version':'110.3.20.7-segment','retargeting_mode':'fullbody_segment_frame_reference_skel_skeleton_layer','scale':float(scale.cpu()),
...
def _build_skel_skin_sequence(torch, model, seq, progress_cb=None):
    ...
    return {
        "version":str(seq.get("version","110.3.20.7-segment")),
...
def _mesh_sequence_npz_bytes(mesh_seq):
    ...
    payload={
        ...
        "skeleton_layer_version":np.asarray(["functional_skel_v1"]),
        "skeleton_layer_semantics":np.asarray(["SKEL joint-chain functional skeleton; visual layer, not patient-specific CT bone geometry"]),
    }
...
def _plot_skel_mesh_walker(mesh_seq):
    ...
    try:
        ...
        if Jviz is not None and pelvis_idx is not None and len(Jviz)==len(Vviz):
            ...
            # V110.3.20.0 · orientación VISUAL fija desde frame 1:
        ...
        payload=_json.dumps({"V":vv,"F":ff,"J":jj,"ids":frame_ids,"edges":edges,"ranges":ranges,"joint_names":names},separators=(",",":"))
        ...
        <script>{plotly_js}</script></head><body><div id='bar'><button onclick='setLayer(\"skin\")'>🧍 Piel</button><button onclick='setLayer(\"skeleton\")'>🦴 Esqueleto</button><button onclick='setLayer(\"both\")'>🧍+🦴 Ambos</button><span style='width:8px'></span><button onclick='playMesh(0.5)'>▶ 0.5×</button><button onclick='playMesh(1)'>▶ 1×</button><button onclick='playMesh(2)'>▶ 2×</button><button onclick='pauseMesh()'>⏸ Pausa</button><button onclick='recordMeshVideo()'>⬇ Vídeo SKEL</button><span id='recstatus'></span><span id='counter'>Frame 1/{len(frame_ids)}</span><input id='slider' type='range' min='0' max='{max(0,len(frame_ids)-1)}' value='0' step='1'></div><div id='plot'></div>
        <script>
        ...
        const data=[{{type:'mesh3d',x:a[0],y:a[1],z:a[2],i:fi,j:fj,k:fk,name:'Piel SKEL',opacity:.72,flatshading:false,lighting:{{ambient:.55,diffuse:.75,specular:.15,roughness:.75}},visible:true}},{{type:'scatter3d',mode:'lines',x:j[0],y:j[1],z:j[2],name:'Esqueleto SKEL',line:{{width:10}},hoverinfo:'skip',visible:true}},{{type:'scatter3d',mode:'markers',x:(P.J.length?P.J[0].map(p=>p[0]):[]),y:(P.J.length?P.J[0].map(p=>p[1]):[]),z:(P.J.length?P.J[0].map(p=>p[2]):[]),text:P.joint_names,hovertemplate:'%{{text}}<extra></extra>',name:'Articulaciones SKEL',marker:{{size:4}},visible:true}}];
        const layout={{title:'V110.3.20.7 SEGMENT · SKEL · Piel + capa esquelética funcional',margin:{{l:0,r:0,t:45,b:0}},scene:{{aspectmode:'data',xaxis:{{range:P.ranges[0]}},yaxis:{{range:P.ranges[1]}},zaxis:{{range:P.ranges[2]}}}},legend:{{orientation:'h'}}}};
        Plotly.newPlot(gd,data,layout,{{displaylogo:false,scrollZoom:true,responsive:true}});
        let layerMode='both'; function setLayer(mode){{layerMode=mode;const skin=(mode==='skin'||mode==='both'),skel=(mode==='skeleton'||mode==='both');Plotly.restyle(gd,{{visible:skin}},[0]);Plotly.restyle(gd,{{visible:skel}},[1,2]);}} function applyFrame(k){{idx=(k+P.V.length)%P.V.length;const a=xyz(idx),j=joints(idx);Plotly.restyle(gd,{{x:[a[0]],y:[a[1]],z:[a[2]]}},[0]);Plotly.restyle(gd,{{x:[j[0]],y:[j[1]],z:[j[2]]}},[1]);if(P.J.length){{const q=P.J[idx];Plotly.restyle(gd,{{x:[q.map(p=>p[0])],y:[q.map(p=>p[1])],z:[q.map(p=>p[2])]}},[2]);}}slider.value=idx;counter.textContent='Frame '+P.ids[idx]+'/'+P.ids[P.ids.length-1];}}
        ...
        st.caption("V110.3.20.7 SEGMENT · visor con capas Piel / Esqueleto / Ambos. La capa esquelética funcional deriva exclusivamente de los joints y cadenas SKEL y no modifica q(t), fitting ni NPZ científico. No representa todavía geometría ósea individual obtenida por imagen médica.")
    except Exception as exc:
...
def render_skel_poc_panel(motion, acquisition_mode="monocular_depth_safe", acquisition_label=None):
    frames=list((motion or {}).get("frames") or [])
    # V110.3.20.0 MINIMAL DIRECT: sin persistencia de secuencia/malla/NPZ.
    if not frames:
        ...
        with st.spinner("Instanciando SKEL y ejecutando el primer forward CPU (1 frame)…"):
            ...
            try:
                ...
                try:
                    ...
                    st.markdown("### V110.3.20.7 SEGMENT · arquitectura segmentaria → SKEL · Piel + Esqueleto")
